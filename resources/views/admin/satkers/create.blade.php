    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('admin.satkers.store') }}" class="space-y-4">
                    @csrf

                    <div>
                        <label for="kode_satker" class="block text-sm font-medium text-gray-700">Kode Satker</label>
                        <input id="kode_satker" name="kode_satker" type="text" value="{{ old('kode_satker') }}"
                    ...
                    <div>
                        ...
                        <input id="nama_satker" name="nama_satker" type="text" value="{{ old('nama_satker') }}"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('nama_satker')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
